...
"""
    ...
    async def poste_list_handler(self, msg):
        ...
        lines = [f"ссылка: {link}\n  чатов: {len(data['chats'])} интервал: {data['interval']} мин" for link, data in poste_list.items()]
    ...
    async def pblk_handler(self, msg):
        ...
        if action == 'list':
            if not poste_blocklist: return await msg.edit("<b>блок-лист пуст</b>", parse_mode='html')
            await msg.edit(f"<b>блок-лист (чаты, куда НЕ отправлять):</b>\n" + "\n".join(str(c) for c in poste_blocklist), parse_mode='html')
        elif action == 'add':
            if len(parts) < 2: return await msg.edit("<b>укажи id чата</b>", parse_mode='html')
            try:
                chat_id = int(parts[1])
                if chat_id not in poste_blocklist: poste_blocklist.append(chat_id)
                await msg.edit(f"<b>чат {chat_id} добавлен в блок-лист</b>", parse_mode='html')
            except ValueError: await msg.edit("<b>id должен быть числом</b>", parse_mode='html')
        elif action == 'del':
            if len(parts) < 2: return await msg.edit("<b>укажи id чата</b>", parse_mode='html')
            try:
                chat_id = int(parts[1])
                if chat_id in poste_blocklist: poste_blocklist.remove(chat_id)
        ...
        else: await msg.edit("<b>неизвестное действие. доступно: list, add, del, clear</b>", parse_mode='html')
...
"""
    ...
    async def selftest_handler(self, msg):
        ...
        except Exception as e: results.append(f"клиент: ОШИБКА ({e})")
        try:
